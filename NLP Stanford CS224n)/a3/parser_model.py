...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        batch_size,n_features = w.shape # (batch_size, n_features)
        w_1d = w.flatten() # (batch_size*n_features,)
        x_2d = torch.index_select(self.embeddings, 0, w_1d) # (batch_size*n_features, embed_dim)
        x = x_2d.view(batch_size,-1)
    ...
    def forward(self, w):
        ...
        x = self.embedding_lookup(w).unsqueeze(1)  # (batch_size, 1, n_features*embed_dim)
        h = nn.functional.relu(torch.matmul(x, self.embed_to_hidden_weight)+self.embed_to_hidden_bias) # (batch_size, 1, hidden_dim)
        l = torch.matmul(h, self.hidden_to_logits_weight)+self.hidden_to_logits_bias # (batch_size, 1, n_classes)
        logits = l.squeeze()
        ### END YOUR CODE
        return logits
