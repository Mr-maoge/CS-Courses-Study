...
def minibatch_parse(sentences, model, batch_size):
    ...
    unfinished_idxs = list(range(n))
    while len(unfinished_idxs) > 0:
        batch_idxs = unfinished_idxs[:batch_size]
        batch_trainsitions = model.predict([parsers[idx] for idx in batch_idxs])
        for idx, transition in zip(batch_idxs, batch_trainsitions):
            p = parsers[idx]
            p.parse_step(transition)
            if len(p.stack)==1 and len(p.buffer) == 0:
                unfinished_idxs.remove(idx)

    dependencies = [parser.dependencies for parser in parsers]
    ### END YOUR CODE
