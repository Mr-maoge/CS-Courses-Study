...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        b = source_padded.shape[1] # batch_size
        X = self.model_embeddings.source(source_padded)  # (src_len, b, embed_size)
        X = pack_padded_sequence(X,source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X) # enc_hiddens: pack_padded_sequence,
                                                                # last_hidden, last_cell: (2, b, hidden_size)
        last_hidden = last_hidden.permute(1,0,2).reshape(b, -1)
        last_cell = last_cell.permute(1,0,2).reshape(b, -1)
        enc_hiddens,_ = pad_packed_sequence(enc_hiddens)       # enc_hiddens (src_len, b, hidden_size*2)
        enc_hiddens = enc_hiddens.permute(1,0,2)    # enc_hiddens (b, src_len, hidden_size*2)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = enc_hiddens_proj.bmm(dec_hidden.unsqueeze(-1)).squeeze(-1)  # (b, src)  [enc_hiddens_proj: (b, src, h) ]
        ...
        alpha_t = F.softmax(e_t, dim=-1)  # (b, src)
        a_t = alpha_t.unsqueeze(1).bmm(enc_hiddens).squeeze(1) # (b, 2*h)
